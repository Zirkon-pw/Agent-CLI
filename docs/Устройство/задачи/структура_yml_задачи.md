...
validation:
  ...
  max_retries: 3  # only for full mode, default 3
  commands:
    - dotnet build
    - dotnet test
```
...
`validation` описывает проверки, которые система запускает после выполнения задачи.

Поддерживаются два режима:

- **simple** -- запуск validation-команд, exit code 0 означает успех, non-zero означает ошибку. Повторных попыток нет.
- **full** -- запуск validation-команд, при ошибке результаты передаются агенту для исправления. Повторные попытки выполняются до `max_retries` раз (по умолчанию 3).
...
Если в проекте принято хранить все автотесты отдельно, validation-команды должны ссылаться именно на директорию `tests/`, а не на тестовые файлы рядом с production-кодом.
